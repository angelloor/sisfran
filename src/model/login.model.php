    ...
    class Login{
        ...
        public function consultarUsuario($usuario, $clave){
            ...
            if($usuarioExiste['count(*)'] == 0 ){
                ...
                $stmt = $connection->prepare("select u.id_usuario, p.nombre_persona, u.nombre_usuario, u.clave, ru.nombre_rol_usuario from usuario u inner join persona p on u.persona_id = p.id_persona inner join rol_usuario ru on u.rol_usuario_id = ru.id_rol_usuario where nombre_usuario = :usuario");
                $stmt->bindValue(":usuario",$usuario,PDO::PARAM_STR); 
                $stmt->execute();
