...
CREATE TABLE `persona` (
  ...
  'TIPO_CONTRATO' char(100) DEFAULT 'LOSEP',
...
CREATE TABLE `permiso` (
  ...
  `DOCUMENTACION_PERMISO` varchar(250),
  `OBSERVACIONES_PERMISO` varchar(250)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `asistencia` (
  ...
  `HORA_ENTRADA_ASISTENCIA` TIME NOT NULL,
  `HORA_SALIDA_ASISTENCIA` TIME NOT NULL,
  `LONGITUD_ASISTENCIA` varchar(100) NOT NULL,
  `LATITUD_ASISTENCIA` varchar(100) NOT NULL,
  `OBSERVACIONES_ASISTENCIA` varchar(250)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
-
-- Filtros MODULO ASISTENCIA
--
ALTER TABLE `permiso`
  ADD CONSTRAINT `permiso_ibfk_1` FOREIGN KEY (`PERMISO_ID`) REFERENCES `permiso` (`ID_PERMISO`) ON DELETE CASCADE ON UPDATE CASCADE;
